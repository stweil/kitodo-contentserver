 ...
 ************************************************************************************/
public class TiffInterpreter extends AbstractImageInterpreter implements
		ImageInterpreter {
	private static final Logger logger = Logger
			.getLogger(TiffInterpreter.class);
	...
	/************************************************************************************
	 * Constructor for {@link TiffInterpreter} to read an tiff image from given
	 * {@link InputStream}
	 * 
	 * @param inStream
	 *            {@link InputStream}
	 ...
	 ************************************************************************************/
	public TiffInterpreter(InputStream inStream)
			throws ImageInterpreterException {
		this.read(inStream);
	}
	 ...
	 * @throws ImageInterpreterException
	 ************************************************************************************/
	protected void read(InputStream inStream) throws ImageInterpreterException {
		...
		if (it.hasNext()) {
			...
			throw new ImageInterpreterException(
					"Imagereader for TIFF format couldn't be found!");
		}
		...
		try {
			...
			tiffDirectory = TIFFDirectory.createFromMetadata(imagereader
					.getImageMetadata(0));
			...
			logger
					.error("something went wrong during reading of jpeg image",
							e);
			throw new ImageInterpreterException(
					"Something went wrong while reading the TIFF from input stream",
					e);
		}
	...
	/**
	 * Retrieves the RenderedImage from the input stream used in the
	 * constructor.
	 * 
	 * @return
	 */
	...
	public void writeToStream(FileOutputStream fos, OutputStream outStream) {
		...
		try {
			...
			BufferedImage bi = ImageManipulator
					.fromRenderedToBuffered(this.renderedimage);
	...
	public boolean pdfBytestreamEmbeddable() {
		if ((this.compressionType == COMPRESSION_NONE)
				|| (this.compressionType == COMPRESSION_LZW)
	...
	public void setWriterCompressionType(int inWriterCompressionType)
			throws ParameterNotSupportedException {
	...
	private void isCompressionApplicable(int inWriterCompressionType)
			throws ParameterNotSupportedException {
		if ((inWriterCompressionType != COMPRESSION_NONE)
				&& (inWriterCompressionType != COMPRESSION_LZW)
				&& (inWriterCompressionType != COMPRESSION_CCITTFAX4)
				&& (inWriterCompressionType != COMPRESSION_PACKBITS)
				&& (inWriterCompressionType != COMPRESSION_JPEG)
				&& (inWriterCompressionType != COMPRESSION_CCITTRLE)) {
			throw new ParameterNotSupportedException(
					"Compression type is not supported");
		}

		// only applicable for black and white
		if ((inWriterCompressionType == COMPRESSION_CCITTFAX4)
				&& ((this.getSamplesperpixel() != 1) || (this.getColordepth() != 1))) {
			throw new ParameterNotSupportedException(
					"Compression type is not supported for this image; only for bitonal images");
		}

		// only applicable for black and white
		if ((inWriterCompressionType == COMPRESSION_CCITTRLE)
				&& ((this.getSamplesperpixel() != 1) || (this.getColordepth() != 1))) {
			throw new ParameterNotSupportedException(
					"Compression type is not supported for this image; only for bitonal images");
		}
