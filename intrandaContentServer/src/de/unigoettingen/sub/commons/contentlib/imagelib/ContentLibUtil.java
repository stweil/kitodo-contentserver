...
/************************************************************************************
 * The Class Helper for some repeating simple tasks like
 * getExtensionFromFileName etc.
 * 
 * @version 09.01.2009 
...
public class ContentLibUtil {
	...
	/************************************************************************************
	 * replace variables in default file names with dynamic content (eg. date
	 * and time)
	 * 
	 * @param inDefaultName
	 *            file name as String including variables
	 * @param inExtension
	 *            file extension that should be added at the end
	 * @return the customized file name as string
	 ************************************************************************************/
	public static String getCustomizedFileName(String inDefaultName,
			String inExtension) {
		if (inDefaultName.contains("$datetime")) {
			SimpleDateFormat sd = new SimpleDateFormat("yyyyMMdd_HHmmss");
			inDefaultName = inDefaultName.replaceAll("\\$datetime", sd
					.format(new Date()));
		}
		return inDefaultName + inExtension;
	}
