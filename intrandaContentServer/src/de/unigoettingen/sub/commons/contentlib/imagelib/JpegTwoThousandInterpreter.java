 ...
 ************************************************************************************/
public class JpegTwoThousandInterpreter extends AbstractImageInterpreter
		implements ImageInterpreter {
	private static final Logger logger = Logger
			.getLogger(JpegTwoThousandInterpreter.class);
	...
	/************************************************************************************
	 * Constructor for {@link JpegTwoThousandInterpreter} to read an jp2 image
	 ...
	 ************************************************************************************/
	public JpegTwoThousandInterpreter(InputStream inStream)
			throws ImageInterpreterException {
		...
		if (it.hasNext()) {
			...
			throw new ImageInterpreterException(
					"Imagereader for Jpeg2000 format couldn't be found!");
		}
		...
		try {
			...
			logger.error(
					"something went wrong during reading of JPEG2000 image", e);
			throw new ImageInterpreterException(
					"Something went wrong while reading the JPEG 2000 from input stream",
					e);
		}
		...
		try {
			...
			try {
				float hps = Float.parseFloat(this
						.getHorizontalPixelSize(domNode));
				float vps = Float
						.parseFloat(this.getVerticalPixelSize(domNode));
			...
			if (samplesperpixel_str != null) {
				...
				if (singlechannels.equals("1 1 1")) {
					...
					int maxint = 0;
					for (int i = 0; i < singlechannels.length; i++) {
						try {
							int s = Integer.parseInt(singlechannels[i]);
							if (s > maxint) {
								maxint = s;
							}
						} catch (Exception e) {
	...
	/**
	 * Bytestream from JPEG 2000 file can be embedded into PDF directly without
	 * recompression
	 */
	...
	public void writeToStream(FileOutputStream fos, OutputStream outStream) {
		...
		try {
			...
			Iterator<ImageWriter> writerIter = ImageIO
					.getImageWritersByFormatName("jpeg2000");
			ImageWriter writer = writerIter.next(); // get writer from ImageIO

			// create metadata by creating an XML tree
			//
			BufferedImage image = ImageManipulator
					.fromRenderedToBuffered(renderedimage);
			...
			if (myWriterCompressionType==LOSSLESS){
				...
				float comprRate=((float)myWriterCompressionValue)/100f;
			...
			ImageTypeSpecifier its = new ImageTypeSpecifier(image
					.getColorModel(), image.getSampleModel());
	...
	/**
	 * sets the metadata for the "javax_imageio_1.0" xml tree. A new tree is
	 * created an later mergered with the already existing one (the default
	 * tree)
	 * 
	 * @param iomd
	 */
	private void setMetadata(IIOMetadata iomd) {
		...
		try {
			...
			Element hpsElement = xmlDocument
					.createElement("HorizontalPixelSize");
	 ...
	 * 
	 * @param domNode
	 * @return returns a string giving the number of samples for each channel
	 *         (e.h. "1 1 1" if it's bitonal).
	 */
	private String getBitsPerSample(Node domNode) {
		...
		Node bitsperSampleNode = getFirstElementByName(datanode,
				"BitsPerSample");
	...
	private String getHorizontalPixelSize(Node domNode) {
		...
		Node horizontalpixelNode = getFirstElementByName(dimensionnode,
				"HorizontalPixelSize");
	...
	private String getVerticalPixelSize(Node domNode) {
		...
		Node verticalpixelnode = getFirstElementByName(dimensionnode,
				"VerticalPixelSize");
