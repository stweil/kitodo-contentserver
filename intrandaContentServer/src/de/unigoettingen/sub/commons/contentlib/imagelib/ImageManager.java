...
public class ImageManager {
	...
	public ImageManager(URL url) {
		init(url, httpproxyhost, httpproxyport, httpproxyuser,
				httpproxypassword);
	}
	 ...
	 */
	public ImageManager(URL url, String httpproxyhost, String httpproxyport,
	 ...
	 * @param httpproxypassword
	 */
	public void init(URL url, String httpproxyhost, String httpproxyport,
			String httpproxyuser, String httpproxypassword) {
		try {
			myInterpreter = ImageFileFormat.getInterpreter(url, httpproxyhost,
	...
	public ImageManager(URL url, HashMap<Integer, Integer> inParameters) {
		...
		init2(url, inParameters, httpproxyhost, httpproxyport, httpproxyuser,
				httpproxypassword);
	}
	 ...
	 */
	public ImageManager(URL url, HashMap<Integer, Integer> inParameters,
			String httpproxyhost, String httpproxyport, String httpproxyuser,
	 ...
	 * @param httpproxypassword
	 */
	public void init2(URL url, HashMap<Integer, Integer> inParameters,
			String httpproxyhost, String httpproxyport, String httpproxyuser,
			String httpproxypassword) {
	 ...
	 */
	public RenderedImage scaleImageByPixel(int pixelx, int pixely, int scaleby,
			int angle) throws ImageManipulatorException {
	...
	public RenderedImage scaleImageByPixel(int pixelx, int pixely, int scaleby,
			...
			throws ImageManipulatorException {
		return scaleImageByPixel(pixelx, pixely, scaleby, 0, coordinates,
				inColor);
	}
	 ...
	 */
	public RenderedImage scaleImageByPixel(int pixelx, int pixely, int scaleby,
			int angle, LinkedList<String> coordinates, Color inColor)
			throws ImageManipulatorException {

		return scaleImageByPixel(pixelx, pixely, scaleby, angle, coordinates,
				inColor, null, false, 0);
	}
	 ...
	 * @throws ImageManipulatorException
	 */
	public RenderedImage scaleImageByPixel(int pixelx, int pixely,
			int externalscalemethod, int angle, LinkedList<String> coordinates,
			Color inColor, Watermark inWatermark, boolean watermarkscale,
			int watermarkposition) throws ImageManipulatorException {
		...
		if ((angle < 0) || (angle > 360)) {
			throw new ImageManipulatorException(
					"Invalid value for angle - must be between 0 and 360 degrees");
		}

		if ((angle > 0) && (coordinates != null) && (coordinates.size() > 0)) {
			throw new ImageManipulatorException(
					"Can't rotate angle, when box cordinates are used!");
		}
		...
		if (coordinates != null) {
			draw_coordinates = ImageManipulator.scaleCoordinates(coordinates,
					internalScaling_x, internalScaling_y);
		}

		// scale image depending on mechanism
		// mechanism depends on color depth
		if ((myInterpreter.getSamplesperpixel() == 1)
				&& (myInterpreter.getColordepth() == 1)) {
			// it's bitonal
			scalemethod = parameters.get(BITONALSCALEMETHOD);
		} else if ((myInterpreter.getSamplesperpixel() == 1)
				&& (myInterpreter.getColordepth() > 1)) {
			// it's greyscale
		...
		if (scalemethod == SUBSAMPLETOGREY) {
			outImage = ImageManipulator.scaleSubsampleBinaryToGrey(inImage,
					internalScaling_x, internalScaling_y);
		} else if (scalemethod == BILINEAR) {
			outImage = ImageManipulator.scaleInterpolationBilinear(inImage,
					internalScaling_x, internalScaling_y);
		} else if (scalemethod == NEXTNEIGHBOR) {
			outImage = ImageManipulator.scaleNextNeighbor(inImage, internalScaling_x,
					internalScaling_y);
		} else {
		...
		// check, if we have to deal with the watermarks
		//
		...
		if (inWatermark != null) {
			if (watermarkscale) {
				...
				if ((watermarkposition == ImageManager.TOP)
					...
					internalScaling_x = (float) outImage.getWidth()
							/ (float) watermarkRi.getWidth();
					internalScaling_y = internalScaling_x;
				} else {
					internalScaling_y = (float) outImage.getHeight()
							/ (float) watermarkRi.getHeight();
					internalScaling_x = internalScaling_y;
				}

				logger.debug("Scaling watermark: image size:"
						+ outImage.getWidth() + " / " + outImage.getHeight()
						+ "   watermark size:" + watermarkRi.getWidth() + " / "
						+ watermarkRi.getHeight() + "\n scalefactor:"
						+ internalScaling_x);

				// scale watermark
				watermarkRi = ImageManipulator.scaleNextNeighbor(watermarkRi,
						internalScaling_x, internalScaling_y);
			...
			logger.debug("Watermark size is: " + watermarkRi.getWidth() + " / "
					+ watermarkRi.getHeight());

			// add renderedImage of Watermark to outImage
			if (watermarkposition == ImageManager.RIGHT) {
				outImage = ImageManipulator.mergeImages(outImage, watermarkRi,
						MergingMode.HORIZONTALLY);
			} else if (watermarkposition == ImageManager.LEFT) {
				outImage = ImageManipulator.mergeImages(watermarkRi, outImage,
						MergingMode.HORIZONTALLY);
			} else if (watermarkposition == ImageManager.TOP) {
				outImage = ImageManipulator.mergeImages(watermarkRi, outImage,
						MergingMode.VERTICALLY);
		...
		if (outImage == null) {
			throw new ImageManipulatorException(
					"Can't create a RenderedImage object, outImage is null");
		}
		return outImage;
	}
