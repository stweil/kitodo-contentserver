...
public class PDFTitlePage {
	...
	LinkedList<PDFTitlePageLine> allTitleLines = new LinkedList<PDFTitlePageLine>();
	LinkedList<PDFTitlePageParagraph> allParagraphs = new LinkedList<PDFTitlePageParagraph>();
	LinkedList<PDFTitlePageImage> allImages = new LinkedList<PDFTitlePageImage>();
	 ...
	 * 
	 * @param inUrl
	 *            Url of xml file for configuration
	...
	public void readConfiguration(URI inUrl) throws PDFManagerException {
		...
		try {
			...
			if (!topmostelement.getNodeName().equals("pdftitlepage")) {
				...
				PDFManagerException pdfe = new PDFManagerException(
						"Response received for PDF's title page configuration file doesn't start with <pdftitlepage>.");
				throw pdfe;
			}
	...
	public void render(com.lowagie.text.Document pdfDoc)
		...
		try {
			// set fonts
			Font smallFont = getPDFFont("Helvetica", 8);
			...
			while (it_image.hasNext()) {
				PDFTitlePageImage pdftpi = (PDFTitlePageImage) it_image.next();
	...
	private void renderTextLine(PDFTitlePageLine inTextLine, Font pdffont,
		...
		if (content.length() > inTextLine.getLinewrap()) {
			...
			String ch1 = ""; // initizilze string (contain all words for one
			// line
			...
			for (int i = 0; i < numberofwords; i++) {
				if ((charsinline + (allwords[i].length()) + 1) < inTextLine
					...
					p1 = new Paragraph(ch1, pdffont); // create a new
					// paragraph
			...
			p1 = new Paragraph(ch1, pdffont); // create a new paragraph
	 ...
	 * 
	 * @param pdftpp
	 *            given {@link PDFTitlePageParagraph} to render
	 * @param pdfdoc
	 ...
	 * 
	 * @param pdftpl
	 *            the given {@link PDFTitlePageLine} to add
	 ************************************************************************************/
	 ...
	 * 
	 * @param pdftpp
	 *            the given {@link PDFTitlePageParagraph} to add
	 ************************************************************************************/
	 ...
	 * 
	 * @param pdftpi
	 *            the given {@link PDFTitlePageImage} to add
	 ************************************************************************************/
	...
	/**
	 * @param termsandconditionstitle
	 *            the termsandconditionstitle to set
	 */
