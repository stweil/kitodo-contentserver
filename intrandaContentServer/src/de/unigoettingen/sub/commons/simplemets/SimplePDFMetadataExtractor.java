...
public class SimplePDFMetadataExtractor implements MetadataExtractor {
	...
	private String pdf_titlepage_line1;
	private String pdf_titlepage_line2;
	private String pdf_titlepage_line3;
	private String pdf_titlepage_line4;

	// define standard value
	// these values are used for comparing the MODS sections with METS
	private String structtype = null;
	private String mods_roleterm_type = null;
	private String mods_roleterm_authority = null;
	private String mods_roleterm_authorvalue = null;
	private String mods_roleterm_editorvalue = null;
	private String mods_roleterm_translator = null;
	private String mods_roleterm_creatorvalue = null;
	private String mods_identifier_type = null;
	 ...
	 * (non-Javadoc)
	 * 
	 * @seede.unigoettingen.sub.gdz.goobi.contentServlet.MetadataExtractor#
	 * calculatePDFMetadata(gov.loc.mets.DivType,
	...
	public void calculateMetadata(DivType inDiv, METSParser metsparser) throws MetsException {
		...
		if (uppermostlogicalDiv.equals(inDiv)) {
			...
			if ((physDivList == null) || (physDivList.size() == 0)) {
			...
			if (parentDiv == null) {
				...
				if (metspointer.size() > 0) {
					// its a volume; there must only be a single metspointer
					// element
					this.calculateLinesForVolume(inDiv, metspointer.get(0), metsparser);
				} else {
	...
	protected void extractTitle() {
		...
		XmlObject[] objects = (XmlObject[]) mods.selectPath(path);

		for (int i = 0; i < objects.length; i++) {
			TitleInfoType titleinfo = (TitleInfoType) objects[i];
	...
	/**
	 * extracts the places of publication; takes only non-encoded (text based)
	 * information If information is not available, an empty string is returned.
	...
	protected String extractPlacePublication() {
		...
		for (int i = 0; i < objects.length; i++) {
			PlaceTermType placeterm = (PlaceTermType) objects[i];
	...
	/**
	 * extract the date of publication in the original form.
	 * &lt:mods:originInfo&gt;/&lt;mods:dateIssued&gt; is extracted independ of
	 * any The characters "[" and "]" are deleted if the value of the node
	 * starts/ends with them.
	 * 
	 * @return
	 */
	protected String extractDatePublication() {
		...
		for (int i = 0; i < objects.length; i++) {
			DateType placeterm = (DateType) objects[i];
			String nodecontents = getObjectContents(placeterm);
	...
	protected void extractCreator() {
		...
		for (int i = 0; i < objects.length; i++) {
			...
			NameType name = (NameType) objects[i];
			...
			for (RoleType rtype : rtypes) {
				...
				for (RoleTerm rterm : rterms) {
					...
					} else if ((code.equals(this.mods_roleterm_type)) && (authority.equals(this.mods_roleterm_authority))) {
						// get value of roleterm
						String role = rterm.getStringValue();

						// check if this role is used as a creator
						if (role.equalsIgnoreCase(this.mods_roleterm_authorvalue)) {
							isauthor = true;
						}
						if (role.equalsIgnoreCase(this.mods_roleterm_translator)) {
							istranslator = true;
						}
						if (role.equalsIgnoreCase(this.mods_roleterm_editorvalue)) {
							iseditor = true;
						}
						if (role.equalsIgnoreCase(this.mods_roleterm_creatorvalue)) {
							iscreator = true;
						}
					}
	...
	protected void extractKeywords() {
		...
		String queryExpression = "//mods:identifier[@type=\"" + mods_identifier_type + "\"]";
		String path = mods_namespaceDeclaration + " " + queryExpression;
		XmlObject[] objects = (XmlObject[]) mods.selectPath(path);

		for (int i = 0; i < objects.length; i++) {
			IdentifierType identifier = (IdentifierType) objects[i];
			if (keywords == null) {
	...
	/**
	 * fills the pdf_titlepage_lines 1-4 with sensible values for a Monograph or
	 * Monograph-like &lt;div&gt; elements
	 * 
	 * first line contains the main title second line contains the author third
	 * line contains publisher and year of publication
	 */
	private void calculateLinesForMonograph() {
		...
		this.pdf_titlepage_line1 = title; // maybe we should shorten the title
		if (author != null) {
			this.pdf_titlepage_line2 = "by: " + author; // author doesn't need
			// to be
		}
		// shortened
		if (place.equals("") && (!date.equals(""))) {
			this.pdf_titlepage_line3 = "published in " + date;
		} else if ((!place.equals("")) && (date.equals(""))) {
			this.pdf_titlepage_line3 = place;
		} else {
			this.pdf_titlepage_line3 = place + "; " + date;
		}
	}

	/**
	 * retireves the parent METS file. The file's location is stored in the Mptr
	 * element as a URL (file or http url).
	 * 
	 * @param inMptr
	 * @return
	 */
	...
	/**
	 * line1: title of multivolume, if not available, title of the volume line2:
	 * volume number line3: author(s) of the multivolume; if not available,
	 * authors of the volume line4: year, publisher and location of volume, if
	 * not available of multivolume
	 * 
	 * @param inDiv
	 * @param inMpts
	 * @param inParser
	 */
	private void calculateLinesForVolume(DivType inDiv, Mptr inMptr, METSParser inParser) {
		...
		if (spme.getTitle() == null) {
			this.pdf_titlepage_line1 = this.extractMainTitle(); // maybe we
			// should shorten the
			// title
		} else {
			String myTitle = spme.getTitle();
			this.pdf_titlepage_line1 = myTitle;
			this.pdftitle = myTitle;
		}

		// line 2 contains the volume number
		if (volumenumber == null) {
			pdf_titlepage_line2 = "unknown volume";
		} else {
			pdf_titlepage_line2 = "volume: " + volumenumber;
		}
		...
		if ((this.extractAuthor() == null) || (this.extractAuthor().equals(""))) {
			...
			if ((author == null) || (author.equals(""))) {
				...
				this.pdf_titlepage_line3 = "by unknown author";
				this.pdfcreator = "unknown author";
			} else {
				// set author from parent (multivolume)
				this.pdf_titlepage_line3 = "by: " + author; // author
				this.pdfcreator = author;
			}
		} else {
			// set author of volume
			this.pdf_titlepage_line3 = "by: " + this.extractAuthor(); // author
		...
		if (place.equals("") && (!date.equals(""))) {
			this.pdf_titlepage_line4 = "published in " + date;
		} else if ((!place.equals("")) && (date.equals(""))) {
			this.pdf_titlepage_line4 = place;
		} else {
			this.pdf_titlepage_line4 = place + "; " + date;
		}
	}

	/**
	 * line1: title of chapter/article line2: if available, author of
	 * chapter/article; otherwise of parent line3: title of parent; if available
	 * volume number of parent; page numbers line4: date and location of
	 * publication
	 * 
	 * @param inDiv
	 * @param myMETSParser
	...
	private void calculateLinesForChapter(DivType inDiv, METSParser myMETSParser) {
		...
		this.pdf_titlepage_line1 = chaptertitle;

		if ((chapterauthor == null) || (chapterauthor.equals(""))) {
			this.pdf_titlepage_line2 = "by " + parentauthor;
			this.pdfcreator = parentauthor;
		} else {
			this.pdf_titlepage_line2 = "by " + chapterauthor;
			this.pdfcreator = chapterauthor;
		}

		// line 3
		if ((parenttitle != null) && (parenttitle.length() > 80)) {
			parenttitle.substring(0, 80);
		}

		this.pdf_titlepage_line3 = "in: " + parenttitle;
		if ((parentvolumenumber != null) && (!parentvolumenumber.equals(""))) {
			this.pdf_titlepage_line3 = "; volume:" + parentvolumenumber;
		}
		this.pdf_titlepage_line3 = this.pdf_titlepage_line3 + ", (page(s) " + startpagenumber + " - " + endpagenumber + ")";

		// line 4
		if (parentplace.equals("") && (!parentyear.equals(""))) {
			this.pdf_titlepage_line4 = "published in " + parentyear;
		} else if ((!parentplace.equals("")) && (parentyear.equals(""))) {
			this.pdf_titlepage_line4 = parentplace;
		} else {
			this.pdf_titlepage_line4 = parentplace + "; " + parentyear;
		}
	}
	 ...
	 * 
	 * @see
	 * de.unigoettingen.sub.gdz.goobi.contentServlet.MetadataExtractor#getPdftitle
	 * ()
	 */
	 ...
	 * 
	 * @see
	 * de.unigoettingen.sub.gdz.goobi.contentServlet.MetadataExtractor#getPdfcreator
	 * ()
	 */
	 ...
	 */
	public String getPdf_titlepage_line1() {
		return pdf_titlepage_line1;
	}
	 ...
	 */
	public String getPdf_titlepage_line2() {
		return pdf_titlepage_line2;
	}
	 ...
	 */
	public String getPdf_titlepage_line3() {
		return pdf_titlepage_line3;
	}
	 ...
	 */
	public String getPdf_titlepage_line4() {
		return pdf_titlepage_line4;
	}
	 ...
	 * 
	 * @param inObject
	 *            {@link XmlObject} with content to get
	 **************************************************************************/
			...
			for (RoleType rtype : ryptes) {
				...
				for (int y = 0; y < objects.length; y++) {
					...
					if ((code == null) || (authority == null)) {
						...
						// check if this role is used as a creator
						if (role.equals(this.mods_roleterm_authorvalue)) {
							return true;
						}
					}
				}
	...
	public void activateGDZConfiguration() {
		...
		// MODS roleterm
		this.mods_roleterm_type = "code";
		this.mods_roleterm_authority = "gdz";
		this.mods_roleterm_authorvalue = "aut";
		this.mods_roleterm_editorvalue = "edt";
		this.mods_roleterm_translator = "tra";
		this.mods_roleterm_creatorvalue = "cre";

		// MODS identifier type
		this.mods_identifier_type = "GDZ";
	}

	/***************************************************************************
	 * set here the configuration for DFG Viewer compatibility for mods
	 **************************************************************************/
	public void activateDFGConfiguration() {
		// roleterm
		this.mods_roleterm_type = "code";
		this.mods_roleterm_authority = "marcrelator";
		this.mods_roleterm_authorvalue = "AUT";
		this.mods_roleterm_editorvalue = "EDT";
		this.mods_roleterm_translator = "TRA";
		this.mods_roleterm_creatorvalue = "CRE";
